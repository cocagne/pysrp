...
if platform == 'darwin':
    ...
    for d in ('libcrypto-1_1-x64.dll',):
        try:
            ...
            print(f'failed loading {d}')
            pass
        try:
            dlls.append( ctypes.cdll.LoadLibrary(os.path.join(r'C:\Users\elyas\Documents\GitHub\pysrp', d)))
        except:
            pass
else:
    try:
...
class BigNumber():
    def __init__(self, *, hexstr : bytes = None, srcbytes : bytes = None, ctx : BigNumberCtx = None):
